...
def min_conflicts(csp, graph, colors, max_iters):
    ...
    raise Exception("Try with more colors or iteractions.")

def main(csp, colors, max_iters):
    start_time = time.process_time()
    graph = create_empty_graph(csp)
    set_random_colors(graph, colors)
    min_conflicts(csp, graph, colors, max_iters)
...
main(generate_random_map(10), generate_colors_list(5), 1000)