...
def random_resolution(csp, max_iters):
    for k in range(max_iters):
        ...
        if len(conflicts_list) == 0:
            ...
            return csp
        random_var = random.randint(0, len(csp) - 1)
        min_conflicts_value = random.randint(0, len(csp) - 1)
...
def main(dim, max_iters):
    ...
    matrix = random_resolution(matrix, max_iters)
